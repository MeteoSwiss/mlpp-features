...
def test_sign_distance_alpine_ridge(alpine_ridge, diagonal_ridge, horizontal_ridge):
    ...
    for ridge in [diagonal_ridge, horizontal_ridge]:
        ...
        assert all(
            np.sign(exp.distances_points_to_line(test_stations_north, ridge)) == 1.0
        ), f"{np.sign(exp.distances_points_to_line(test_stations_north, ridge))}, {ridge[0]}"

        assert all(
            np.sign(exp.distances_points_to_line(test_stations_south, ridge)) == -1.0
        ), f"{np.sign(exp.distances_points_to_line(test_stations_south, ridge))}"

        assert all(
            np.sign(exp.distances_points_to_line(ridge, ridge)) == 0.0
    ...
    assert all(
        np.sign(exp.distances_points_to_line(test_stations_alpine_ridge, alpine_ridge))
        == [-1, 1, 1, -1, -1]
