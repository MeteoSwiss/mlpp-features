...
def heat_index_ensavg(
    ...
    t_celsius = temperature_ensavg(data, stations, reftimes, leadtimes, ds, **kwargs)
    u = relative_humidity_ensavg(data, stations, reftimes, leadtimes, ds, **kwargs)
...
@reuse
@asarray
def leadtime(data: Dict[str, xr.Dataset], stations, reftimes, leadtimes, ds, **kwargs):
    """
...
def relative_humidity_ensavg(
    ...
    """
    e = water_vapor_pressure_ensavg(data, stations, reftimes, leadtimes, ds, **kwargs)
    e_s = water_vapor_saturation_pressure_ensavg(
        data, stations, reftimes, leadtimes, ds, **kwargs
    )
...
def sx_500m(
    ...
    wdir = (
        wind_from_direction_ensavg(data, stations, reftimes, leadtimes, ds, **kwargs)
        .astype("int16")
        .load()
    )
...
def water_vapor_mixing_ratio_ensavg(
    ...
    try:
        q = specific_humidity_ensavg(data, stations, reftimes, leadtimes, ds, **kwargs)
        return (q / (1 - q)).astype("float32")
    except KeyError:
        e = water_vapor_pressure_ensavg(
            data, stations, reftimes, leadtimes, ds, **kwargs
        )
        p = pressure_ensavg(data, stations, reftimes, leadtimes, ds, **kwargs)
...
def water_vapor_pressure_ensavg(
    ...
    dew_point_temperature = dew_point_ensavg(data, stations, reftimes, leadtimes, ds)
    air_temperature = temperature_ensavg(data, stations, reftimes, leadtimes, ds)
...
def wind_speed_error(
    ...
    """
    nwp = wind_speed_ensavg(data, stations, reftimes, leadtimes, ds, **kwargs)
...
def wind_speed_of_gust_error(
    ...
    """
    nwp = wind_speed_of_gust_ensavg(data, stations, reftimes, leadtimes, ds, **kwargs)
